-- Insert the signing key into vault for use by database trigger
-- Note: The actual secret value needs to be inserted manually in Supabase dashboard
-- or we use the environment variable that was just added

-- For now, let's verify the trigger can work without vault by using a fallback
-- We'll update the function to also check environment if vault fails
...
BEGIN
  ...
  IF signing_key IS NULL THEN
    RAISE NOTICE 'Skipping notification: edge_function_signing_key not configured in vault. Please add it via Supabase Dashboard > Project Settings > Vault';
    RETURN NEW;
  END IF;
